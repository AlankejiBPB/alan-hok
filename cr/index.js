window.onload = function () {
    ...
    document.querySelector("body > mdui-layout > mdui-top-app-bar > mdui-top-app-bar-title").innerText = "王者荣耀自定义房间开房工具"
}
...
if (localStorage.getItem("wzzdy_createtip") != "0.2") {
    mdui.alert({
        headline: "提示",
        description: "房主打开游戏训练营选择英雄后 进入加载页时打开网页 点击启动游戏跳转游戏即可解除不满人无法开启的限制 其他人直接使用生成的链接进入房间即可",
        confirmText: "我知道了",
...
function 打开链接(url) {
    ...
    mdui_snackbar({
        message: "启动成功 如没反应请检查是否安装王者荣耀或尝试使用浏览器打开",
        action: "我知道了",
...
function 生成链接(func) {
    ...
    if (isJSON(custom)) {
        ...
        if (customjson[edittab[2].value]) {
            ...
            try {
                ...
                mdui.alert({
                    headline: "提示",
                    description: "合成游戏参数配置时出错 错误信息:" + error,
                    confirmText: "我知道了",
                });
                return
            }
    ...
    if (isNaN(mapname)) {
        ...
        mdui.alert({
            headline: "提示",
            description: "游戏模式获取失败 请选择游戏模式后重新尝试",
            confirmText: "我知道了",
            onConfirm: () => console.log("confirmed"),
        });
        return
    ...
    if (mapname.includes("仅开房")) {
        ...
        var tiptext = "此模式仅提供开房间 不可配置参数 确定启动游戏？"
...
function getShortLink(longUrl) {
    if (longUrl.startsWith('https://') != true) {
        ...
        mdui.snackbar({
            message: "当前非正式环境(https) 生成链接不可用 请将源码放入正式环境中以启用该功能",
            action: "我知道了",
            onActionClick: () => console.log("click action button")
        });
    }
...
function replaceContent(str, replaceurl, replacepos, openurl) {
    ...
    function showcopytipdia() {
        mdui.alert({
            headline: "提示",
            description: "生成规则不能为空 已恢复默认生成规则",
            confirmText: "我知道了",
            onConfirm: () => console.log("confirmed"),
        });
    }
...
function showqr(url, func) {
    ...
    mdui.confirm({
        headline: "王者荣耀房间",
        description: "房间链接:"+url,
        body: '<img>',
        confirmText: "复制链接并打开游戏",
        cancelText: "关闭",
        onConfirm: () => func(),
        onCancel: () => console.log("canceled"),
        onOpen: (dia) => {
...
allbutton[1].onclick = function () {
    ...
    var span = document.createElement('span');
    // 设置<span>元素的文本内容  
    span.innerHTML = '<span slot="description">你可以编辑房间链接的生成规则</span>';
    mdui.dialog({
        headline: "提示",
        description: span,
        actions: [
            ...
            },
            {
                ...
                onClick: () => {
                    ...
                    if (window.openurl) {
                        ...
                        getShortLink(window.location.origin + "/cr/Smoba.html?data=" + openurl)
                            .then(shortLink => {
                                murl = processLink(shortLink);
                                work_message = "null"
                                mdui.confirm({
                                    headline: "提示",
                                    description: "成功获取到房间数据，是否生成链接？",
                                    confirmText: "确认",
                                    cancelText: "取消",
                                    onConfirm: () => {
                                        let url = replaceContent(myedit.value, window.location.origin + "/cr/data.html?" + murl, 0, openurl)
                                        showqr(url, function () {
                                            复制文本(url)
                                        })
                                    },
                            ...
                            .catch(error => {
                                ...
                                var span = document.createElement('span');
                                // 设置<span>元素的文本内容  
                                span.innerHTML = '<span slot="description">尝试使用以下方法解决：<br>1.删除过于离谱的配置 例如禁用全部英雄等<br>2.检查网络是否正常 或者切换至移动数据<br>3.房间创建有冷却 请重试2-3次<br>4.生成规则填写错误 检查规则或使用默认规则</span>';
                                mdui.alert({
                                    headline: "未获取到房间数据",
                                    description: span,
                                    confirmText: "我知道了",
        ...
        body: '<mdui-text-field class="copydialog_edit" variant="filled"   style="padding-top: 10px;" label="生成规则"></mdui-text-field>\n<p><br>当生成规则中有以下字符 会被自动替换为指定字符：<br>mode --> 游戏模式名称<br>hero --> 禁用英雄配置名称<br>ban --> 禁用英雄配置<br>custom --> 游戏参数配置名称<br>url --> 房间链接<br>gametype --> 大区名称<br>\\n --> 换行<br><br>默认规则为url</p>',
        onOpen: (dia) => {
...
allbutton[3].onclick = function () {
    ...
    mdui.confirm({
        headline: "提示",
        description: "是否更新英雄列表 点击继续获取 成功后会自动刷新网页 当之后英雄列表变化可以手动点击更新英雄列表更新",
        confirmText: "继续",
        cancelText: "取消",
        onConfirm: () => {
            get_message = "获取英雄列表中 请耐心等待"
...
for (item in mydatajson[0]) {
    ...
    (function (item_str) {
        ...
        if (item_str.includes("征召")) {
            // 创建 mdui-tooltip 元素
            const tooltip = createTooltip("注意", "征召不可以添加人机");
...
function 检测英雄随机禁用(str) {
    if (str.includes("suiji")) {
        ...
        mdui.alert({
            headline: "提示",
            description: "该禁用英雄配置为随机禁用配置 随机禁用" + num + "个英雄",
            confirmText: "我知道了",
        });
        return true
    }
    ...
    if (localStorage.getItem("custom_heros")) {
        ...
        if (editvalue in JSON.parse(localStorage.getItem("custom_heros"))) {
            ...
            if (heros_json[editvalue].includes("suiji")) {
                mdui.alert({
                    headline: "提示",
                    description: "该禁用英雄配置为随机禁用配置 暂不支持该操作",
                    confirmText: "我知道了",
                });
                return
            }
...
heroButton[9].onclick = function () {
    ...
    mdui.prompt({
        ...
        description: "输入后 将创建一个新的配置 该配置为随机禁用的配置 你可选择随机禁用生效的分路",
        confirmText: "确认",
        cancelText: "取消",
        onOpen: (dia) => {
    ...
    function processNextTask() {
        if (tasks.length > 0) {
            ...
            if (localStorage.getItem("wzzdy_customsettip") != "0.1") {
                mdui.alert({
                    headline: "提示",
                    description: "网页偏重自定义属性 网页为自定义属性提供了高级设置 你可以手动在高级设置中实现更多高级功能 高级设置与该配置绑定 例如 你可以设置随机生成和打乱指定自定义属性 实现属性与地图模式 禁用英雄的绑定 等等",
                    confirmText: "我知道了",
...
function makejson(HeroList, bxList, ygList, fytList, sjList, gjjson) {
    ...
    for (let index = 0; index < jsondo.length; index++) {
        ...
        if (value == "" || value == "0") {
            throw "游戏参数配置信息有误 点击游戏参数的管理配置手动保存配置来更新该配置即可解决该问题"
        }
        if (value == "1") {
...
function showeditdia(textv, func, ele) {
    ...
    let dia = mdui.dialog({
        headline: "提示",
        description: step[0],
        body: '<p>如果显示不全可向下滑动查看更多内容</p><div class="radiodiv"></div>',
        onOpen: (dia) => {
            let mdui_buttons = dia.querySelectorAll("mdui-button")
            mdui_buttons.forEach(mdui_button => {
                mdui_button.style.margin = "0px"
